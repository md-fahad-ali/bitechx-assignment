...
export default function SearchLoadingBridge() {
  const [searching, setSearching] = useState(false);

  useEffect(() => {
    const onPending = () => setSearching(true);
    const onDone = () => setSearching(false);

    if (typeof window !== "undefined") {
      window.addEventListener("search:pending", onPending as EventListener);
      window.addEventListener("search:done", onDone as EventListener);
    }

    return () => {
      if (typeof window !== "undefined") {
        window.removeEventListener("search:pending", onPending as EventListener);
        window.removeEventListener("search:done", onDone as EventListener);
      }
    };
  }, []);
  ...
  // Inline skeleton grid (mirrors products/loading.tsx grid section)
  return (
