...
async function fetchProducts(opts: { token: string; q?: string; categoryId?: string }) {
  ...
  if (q && q.trim().length > 0) {
    const url = `${base}/products/search?${new URLSearchParams({ searchedText: q }).toString()}`;
  ...
  const res = await fetch(`${base}/products${sp.toString() ? `?${sp}` : ''}`, { headers, cache: 'force-cache', next: { revalidate: 30 } });
  if (!res.ok) {
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!token) {
    return (
      <div className="max-w-3xl mx-auto text-center py-20">
        <h1 className="text-2xl font-semibold text-[var(--nav-fg)]">Please sign in to view products</h1>
        <p className="mt-2 opacity-80">You must be authenticated to access the catalog.</p>
  ...
  const catsFinal: { id: string; name: string }[] = (cats && cats.length)
    ? cats
    : Array.from(
        new Map(
          (items || [])
    ...
    <div className="max-w-7xl mx-auto px-3 md:px-4 lg:px-8 space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="flex items-center justify-center py-20">
          <span className="text-[var(--nav-fg)] opacity-70">No Product Found</span>
        </div>
      ) : (
        <div>
          <ul className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            {items.slice(offset, offset + limit).map((p: Product) => (
            <li key={p.id} className="group rounded-lg border border-[var(--nav-border)] bg-[var(--nav-bg)] overflow-hidden">
              <div className="block">
                <div className="relative aspect-[4/3] bg-[color:rgba(13,24,33,0.05)] dark:bg-[color:rgba(239,241,243,0.06)]">
                  ...
                  {(() => {
                    ...
                    const valid = typeof first === 'string' && /^(https?:)?\/\//i.test(first);
                    return valid ? (
                      <img src={first} alt={p.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
                    ) : (
